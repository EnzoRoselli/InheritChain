...
contract InheritanceFactory {
    ...
    event LogInheritanceCreated(
        string message,
        address indexed administrator,
        address indexed inheritance,
        uint256 deployedInheritancesCount
    );

    function createInheritance(
        uint256 aliveTimeOut,
        address usdcTokenAddress
    ) public {
        require(
            inheritances[msg.sender] == address(0),
            "You have already one inheritance created, you can't have two."
        ); // address(0) means null address in Solidity.

        inheritances[msg.sender] = address(
            new Inheritance(msg.sender, aliveTimeOut, usdcTokenAddress)
        );
        deployedInheritancesCount++;

        emit LogInheritanceCreated(
            "Log all the values: Administrator address, Inheritance contract address, Number of inheritances deployed.",
            msg.sender,
            inheritances[msg.sender],
            deployedInheritancesCount
        );
    }
...
contract Inheritance is Ownable {
    ...
    mapping(uint256 => address) public nftHeirs;
    ...
    event LogInheritanceClaimed(
        string message,
        address indexed heir,
        uint256 share,
        uint256 numberOfHeirs
    );
    event LogRequestToBeHeir(
        string message,
        address indexed requester,
        uint256 numberOfRequests,
        uint256 numberOfHeirs
    );
    event LogHeirAccepted(
        string message,
        address indexed heir,
        uint256 share,
        uint256 numberOfRequests,
    ...
    event LogHeirRejected(
        string message,
        address indexed requester,
        uint256 numberOfRequests,
        uint256 numberOfHeirs
    );
    event LogHeirClaiming(
        string message,
        address indexed heir,
        uint256 share,
        uint256 etherSent,
    ...
    function deposit() public payable {
        emit LogDeposit(
            "Log all the values: Administrator address, Amount sent, Contract ether balance.",
    ...
    function withdrawUSDC(uint256 amount) public onlyOwner {
        ...
        require(
            ...
            "Error while transferring USDC from the contract to the administrator."
        );
    }
    ...
    function signalAlive() public onlyOwner {
        ...
        emit LogAdministratorAlive(
            "Administrator sends a signal to show he is alive.",
            msg.sender,
            administrator.lastAlive,
            block.timestamp,
            aliveTimeOut
        );
    }
    ...
    function requestInheritance(
        address requester
    ) public requestInheritanceValidations(requester) {
        inheritanceRequests.push(requester);
        emit LogRequestToBeHeir(
            "User requests to be added as an heir.",
            requester,
            inheritanceRequests.length,
            heirs.length
        );
    }

    function acceptInheritanceRequest(
        uint index,
        address requester,
        uint256 share
    ) public onlyOwner acceptInheritanceValidations(index, requester, share) {
        uint256[] memory emptyArray;
        heirs.push(Heir(requester, share, 0, 0, emptyArray));
        removeRequest(index);
        emit LogHeirAccepted(
            "Administrator accepts user as an heir.",
            requester,
            share,
            inheritanceRequests.length,
            heirs.length
    ...
    function rejectInheritanceRequest(
        uint index,
        address requester
    ) public onlyOwner rejectInheritanceRequestValidations(index, requester) {
        removeRequest(index);
        emit LogHeirRejected(
            "Administrator rejects user's request to be an heir.",
            requester,
            inheritanceRequests.length,
            heirs.length
        );
    ...
    function isAdministratorDead() public view returns (bool) {
        // Shows if the aliveTimeOut has been reached, and the Heirs can redeem the Inheritance.
    ...
    function claimInheritance() public isHeir(msg.sender) {
        if (isAdministratorDead()) {
            ...
            for (uint i = 0; i < heirs.length; i++) {
                ...
                require(
                    token.transfer(heirAddress, heirsUSDC[heirAddress]),
                    "Error while transferring USDC from the contract to heir."
                );
                heirs[i].usdcBalance = heirsUSDC[heirAddress];

                emit LogHeirClaiming(
                    "Ether and USDC money sent to the heir.",
                    heirAddress,
                    heirs[i].share,
                    heirsEther[heirAddress],
                    heirs[i].etherBalance,
                    heirsUSDC[heirAddress],
                    heirs[i].usdcBalance
                );
            }
        }
    ...
    function addNFTDeed(
        uint256 tokenId,
        address newHeirAddress
    ) public onlyOwner {
        address currentHeir = nftHeirs[tokenId];
        if (currentHeir != address(0)) {
            // Remove the tokenId from the previous heir's NFTDeedIds array
            for (uint256 i = 0; i < heirs.length; i++) {
                if (heirs[i].heir == currentHeir) {
                    uint256 index = indexOfToken(heirs[i].NFTDeedIds, tokenId);
                    removeTokenAtIndex(heirs[i].NFTDeedIds, index);
                    break;
                }
            }
        }

        // Assign the tokenId to the new heir's NFTDeedIds array
        for (uint256 i = 0; i < heirs.length; i++) {
            if (heirs[i].heir == newHeirAddress) {
                heirs[i].NFTDeedIds.push(tokenId);
                nftHeirs[tokenId] = newHeirAddress;
                return;
            }
        }
        require(false, "New heir address not found among heirs.");
    }

    function removeNFTDeed(uint256 tokenId) public onlyOwner {
        address currentHeir = nftHeirs[tokenId];
        require(currentHeir != address(0), "Token not assigned to any heir.");

        // Remove the tokenId from the current heir's NFTDeedIds array
        for (uint256 i = 0; i < heirs.length; i++) {
            if (heirs[i].heir == currentHeir) {
                uint256 index = indexOfToken(heirs[i].NFTDeedIds, tokenId);
                removeTokenAtIndex(heirs[i].NFTDeedIds, index);
        ...
        // Remove the tokenId from the nftHeirs mapping
        delete nftHeirs[tokenId];
    }

    function removeTokenAtIndex(uint256[] storage arr, uint256 index) internal {
    ...
    function indexOfToken(
        ...
        require(false, "Token not found in array.");
    }
    ...
    function getNFTDeedsByHeirAddress(
        address heirAddress
    ) public view returns (uint256[] memory nftDeedIds) {
        for (uint256 i = 0; i < heirs.length; i++) {
    ...
    modifier requestInheritanceValidations(address addr) {
        ...
        require(
            inheritanceRequests.length < 100,
            "Already reached the limit of 100 inheritance requests."
        );

        for (uint i = 0; i < inheritanceRequests.length; i++) {
            require(
                inheritanceRequests[i] != addr,
    ...
    modifier acceptInheritanceValidations(
        uint index,
        address addr,
        uint256 share
    ) {
        require(
            index < inheritanceRequests.length,
            "The index sent is bigger than the requests array length."
        );
        require(
            inheritanceRequests[index] == addr,
        ...
        require(
            ...
            "the amount of shares exceed the 100% of the Inheritance, try with a smaller amount of share."
        );
        _;
    }
    ...
    modifier isHeir(address addr) {
        ...
        require(
            isPresent == true,
            "The address claiming the inheritance is not a Heir."
        );
        _;
    }
